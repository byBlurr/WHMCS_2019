...
namespace WHMCS
{
    public class API
    {
        ...
        public bool AcceptOrder(int OrderId, int ServerId = -1, string ServiceUsername = "", string ServicePassword = "", string Registrar = "", bool SendRegistrar = true, bool AutoSetup = true, bool SendEmail = true)
        {
        ...
        /// <param name="AutoSetup">Send the request to the product module to activate the service. This can override the product configuration. Default: true</param>
        /// <param name="SendEmail">Send any automatic emails. This can be Product Welcome, Domain Renewal, Domain Transfer etc. Default: true</param>
        public async Task AcceptOrderAsync(int OrderId, int ServerId = -1, string ServiceUsername = "", string ServicePassword = "", string Registrar = "", bool SendRegistrar = true, bool AutoSetup = true, bool SendEmail = true)
        {
            NameValueCollection data = new NameValueCollection()
            {
                { "action", APIEnums.Actions.AcceptOrder.ToString() }
            ...
            string req = await _call.MakeCallAsync(data);
            JObject result = JObject.Parse(req);
        ...
        /// <param name="Fees">The amount of the payment that was taken as a fee by the gateway</param>
        /// <param name="NoEmail">Set to true to not send an email for the invoice payment</param>
        public async Task AddInvoicePaymentAsync(int InvoiceId, string TransactionId, string Gateway, string DateTime = "", float Amount = 0f, float Fees = 0f, bool NoEmail = false)
        {
            NameValueCollection data = new NameValueCollection()
            {
                { "action", APIEnums.Actions.AddInvoicePayment.ToString() },
